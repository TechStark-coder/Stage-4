...
export function RoomCard({ room, homeId, homeName, onRoomDeleted, onRoomUpdated }: RoomCardProps) {
  ...
  return (
    <div className="custom-3d-card-wrapper noselect"> {/* Changed class name */}
      <div className="custom-3d-card-canvas"> {/* Changed class name */}
        {/* 25 tracker divs for hover effect */}
        {Array.from({ length: 25 }, (_, i) => (
          <div key={`tracker-${room.id}-${i}`} className={`custom-3d-tracker tr-${i + 1}`}></div>
        ))}
        
        <div className="custom-3d-card-interactive-area"> {/* Changed from id="card" to class */}
            ...
            <div className="flex justify-between items-start mb-1">
              <div className="flex-grow">
                <h3 className="text-lg font-semibold text-white flex items-center gap-2 truncate">
                  <DoorOpen className="h-5 w-5 text-purple-300 flex-shrink-0" />
              ...
              {room.objectNames && room.objectNames.length > 0 && !room.isAnalyzing && (
                 <button 
                    ...
                    // Adjusted class for the download button to avoid conflicting with main 3D card styles
                    // and to ensure it's clickable and positioned well.
                    className="botao-download group !p-1 !w-8 !h-8 !shadow-none !filter-none !bg-transparent hover:!w-8 hover:!h-8 hover:!rounded-md !text-white"
                 ...
                 >
                    {_isDownloading ? (
                      ...
                      <>
                        ...
                        <span className="texto !hidden">Download</span> {/* Hide text initially */}
                      </>
                    )}
                </button>
              )}
          ...
          <div className="flex-grow w-full px-3 py-1 text-sm text-neutral-200 overflow-hidden">
            {room.isAnalyzing ? (
              <p className="flex items-center gap-1 text-xs"> {/* Smaller text */}
                <Loader2 className="h-3 w-3 animate-spin" /> Analyzing...
              </p>
            ) : room.objectNames && room.objectNames.length > 0 ? (
              <p className="line-clamp-3 text-xs"> 
                <span className="font-medium text-neutral-100">Last analysis:</span> {room.objectNames.join(', ').substring(0, 50)}{room.objectNames.join(', ').length > 50 ? '...' : ''}
              </p>
            ) : (
              <p className="text-center text-xs text-neutral-300 italic mt-2">No objects analyzed yet.</p>
            )}
          </div>

          {/* Bottom section: Action Buttons */}
          <div className="w-full p-2 flex justify-between items-center border-t border-white/10 mt-auto">
            <div className="flex gap-1"> 
              <EditRoomDialog room={room} homeId={homeId} onRoomUpdated={onRoomUpdated} />
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button variant="destructive-outline" size="sm" className="!text-red-400 hover:!bg-red-700/20 !border-red-500/50 hover:!border-red-500 !px-2 !h-7"> 
            ...
            <Button asChild variant="ghost" size="sm" className="text-purple-300 hover:text-purple-100 hover:bg-white/10 !px-2 !h-7 !text-xs"> 
